...
const AdminReminders: React.FC<AdminRemindersProps> = ({ appointments, services, settings, onUpdate }) => {
  ...
  const sendReminder = (app: Appointment) => {
    ...
    const message = `Olá ${app.clientName}! ✨\n\nPassando para lembrar do seu agendamento amanhã (${app.date}) às ${app.time} para o serviço de *${srv?.name || 'Beleza'}* no ${settings.salonName}.\n\nPodemos confirmar sua presença? 💖`;
    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-serif font-bold text-slate-800 flex items-center">
          <Bell className="mr-3 text-rose-500" size={32} /> Central de Lembretes
        </h1>
        <p className="text-slate-500">Agendamentos para amanhã ({tomorrowStr}) que precisam de confirmação.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {upcomingAppointments.length > 0 ? (
              ...
              <div key={app.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 group hover:border-rose-200 transition-all">
                ...
                <div className="flex items-center space-x-3 w-full md:w-auto">
                  ...
                  <button 
                    ...
                    className={`flex-1 md:flex-none flex items-center justify-center space-x-2 px-6 py-3 rounded-xl font-bold transition-all shadow-lg ${
                      app.reminderSent 
                  ...
                  >
                    ...
                    <span>{app.reminderSent ? 'Reenviar WhatsApp' : 'Enviar WhatsApp'}</span>
                  </button>
                </div>
              </div>
            );
          ...
          <div className="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-slate-100">
            ...
            <h3 className="text-slate-800 font-bold text-xl mb-1">Nenhum agendamento para amanhã</h3>
            <p className="text-slate-400 max-w-xs mx-auto text-sm">Tudo em dia! Novos lembretes aparecerão aqui automaticamente conforme os agendamentos forem feitos.</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-rose-50 p-6 rounded-3xl border border-rose-100">
        <h4 className="text-rose-800 font-bold mb-2 flex items-center">
          <MessageSquare size={16} className="mr-2" /> Dica de Gestão
        </h4>
        <p className="text-rose-700 text-sm leading-relaxed">
          Enviar lembretes 24h antes reduz o índice de faltas em até 40%. Ao clicar no botão, o sistema abre o WhatsApp do cliente com uma mensagem pronta e marca o agendamento como "Notificado".
        </p>
      </div>
    </div>
  );
