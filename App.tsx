...
import { Menu, X, Settings, LayoutDashboard, Scissors, Users, Calendar, LogOut, ChevronRight, Bell, History, Tag, Home as HomeIcon } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleUpdateCategories = (newCategories: Category[]) => {
    db.saveCategories(newCategories);
    setCategories(newCategories);
  };

  const handleUpdateProfessionals = (newProfessionals: Professional[]) => {
    db.saveProfessionals(newProfessionals);
    setProfessionals(newProfessionals);
  };

  const handleUpdateAppointments = (newAppointments: Appointment[]) => {
    db.saveAppointments(newAppointments);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="flex justify-between items-center">
              ...
              >
                ...
                <span className={`text-2xl font-serif font-bold ml-3 tracking-tight ${
                  view === 'home' && !isScrolled ? 'text-white' : 'text-slate-900'
                }`}>
                  {settings.salonName}
                </span>
              </div>
              ...
              <div className="hidden md:flex space-x-10 items-center">
                <button 
                  onClick={() => navigate('home')} 
                  className={`font-bold text-sm uppercase tracking-[0.2em] transition-all hover:opacity-100 ${
                    view === 'home' && !isScrolled ? 'text-white opacity-80' : 'text-slate-600 opacity-70'
                  }`}
                >
                  Início
                </button>
                <button 
                  ...
                  className={`px-8 py-3.5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-xl hover:-translate-y-1 active:translate-y-0 ${
                ...
                <button 
                  ...
                  className={`p-2 transition-colors ${
                    view === 'home' && !isScrolled ? 'text-white/40 hover:text-white' : 'text-slate-300 hover:text-slate-600'
                  }`}
                >
                  <Settings size={20} />
                </button>
      ...
      <main className={`flex-1 ${isAdminView ? 'lg:bg-[#fafafa]' : ''} ${!isAdminView && view !== 'home' ? 'pt-24' : ''}`}>
        <div className={isAdminView ? "max-w-7xl mx-auto p-6 lg:p-12 pb-24 lg:pb-12" : ""}>
